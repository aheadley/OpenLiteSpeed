...
void HttpVHost::offsetChroot( const char * pChroot, int len )
{
    ...
    if ( m_pLogger )
    {
        ...
        if ( strncmp( pChroot, pOldName, len ) == 0 )
        {
            ...
            int rollSize = m_pLogger->getAppender()->getRollingSize();
            strcpy( achTemp, pOldName + len );
...
void HttpVHost::setErrorLogRollingSize( int size )
{
    if ( m_pLogger )
...
void HttpVHost::setBytesLogFilePath( const char * pPath, long rollingSize )
{
    if ( m_pBytesLog )
    {
        m_pBytesLog->close();
