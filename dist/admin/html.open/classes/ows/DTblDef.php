...
class DTblDef
{
	...
	protected function loadCommonAttrs()
	{
		...
		$this->_attrs = array(
			...
			'rollingSize' => new DAttr('rollingSize', 'uint', 'Rolling Size (bytes)', 'text', true, '1M', NULL, NULL, NULL, 'log_rollingSize'),
			...
			'ext_path' => new DAttr('path', 'file1', 'Command', 'text', true, 3, 'x', $this->_options['text_size']),
	...
	protected function add_VH_CTXC($id)
	{
		...
		$attrs = array(
			$this->_attrs['ctx_uri'],
			$this->_attrs['ctx_order'],
			$this->_attrs['ctx_location']->dup(NULL, 'Path', NULL),
			$this->_attrs['note'],
			$this->_attrs['extraHeaders'],
